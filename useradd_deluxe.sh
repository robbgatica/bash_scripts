...
echo 'Create an account on the local system with name of USER_NAME and a comments field of COMMENT.' >&2
exit 1
fi
...
useradd -c "${COMMENT}" -m ${USER_NAME} &> /dev/null #send both STDOUT & STDERR

# Check the exit code to see if useradd was successful
if [[ "${?}" -ne 0 ]]
then
echo 'There was a problem adding the user' >&2
exit 1
fi
